...
public class ColorPickerTriangle : MonoBehaviour, IPointerClickHandler
{
 ...
 //   }
    ...
    {
        MyPlane = new Plane(transform.TransformDirection(Vector3.forward), transform.position);
        Ray ray = new Ray(eventData.selectedObject.transform.position, eventData.selectedObject.transform.forward);
        float rayDistance;
